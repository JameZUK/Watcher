...
_SYSTEM = (
    ...
    "names a specific qualifier (e.g. 'analyst reviews', 'reviews from managers') but the "
    "actual new item is a different kind (a review from a support engineer or an account "
    "manager), describe it by its REAL details and NEVER attach the instruction's qualifier "
    "to it. A new item that doesn't match the specific qualifier the user asked for is a "
    "weaker match — rate it 'low' and say it isn't the specific kind requested.\n"
    "\n"
    "IMPORTANCE:\n"
    "- high: matches the watch instruction and the user almost certainly wants to know "
    "(or, with no instruction: a price/stock/availability or key-content change).\n"
