...
async def _fleet_summary(session, user, monitors) -> dict:
    ...
    return {
        ...
        "fingerprint": f"{max_id}:{changes}",
    }


async def _generate_fleet_paragraph(user_id, fp, model, base_url, key) -> None:
    ...
    try:
        ...
        segs = await summarize_fleet(api_key=key, model=model, base_url=base_url, changes=changes)
        if segs:
